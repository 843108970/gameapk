...
public class FontsContractCompat {
    ...
    static FontInfo[] getFontFromProvider(Context context, FontRequest fontRequest, String str, CancellationSignal cancellationSignal) {
        String str2 = str;
        ArrayList arrayList = new ArrayList();
        Uri build = new Uri.Builder().scheme(b.W).authority(str2).build();
        Uri build2 = new Uri.Builder().scheme(b.W).authority(str2).appendPath("file").build();
        Cursor cursor = null;
        try {
            Cursor query = Build.VERSION.SDK_INT > 16 ? context.getContentResolver().query(build, new String[]{TrayContract.Preferences.Columns.ID, Columns.FILE_ID, Columns.TTC_INDEX, Columns.VARIATION_SETTINGS, Columns.WEIGHT, Columns.ITALIC, Columns.RESULT_CODE}, "query = ?", new String[]{fontRequest.getQuery()}, (String) null, cancellationSignal) : context.getContentResolver().query(build, new String[]{TrayContract.Preferences.Columns.ID, Columns.FILE_ID, Columns.TTC_INDEX, Columns.VARIATION_SETTINGS, Columns.WEIGHT, Columns.ITALIC, Columns.RESULT_CODE}, "query = ?", new String[]{fontRequest.getQuery()}, (String) null);
