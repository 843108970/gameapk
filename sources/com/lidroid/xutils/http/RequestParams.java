...
public class RequestParams {
    ...
    public class HeaderItem {
        ...
        public HeaderItem(Header header2) {
            this.overwrite = false;
            this.header = header2;
        }

        public HeaderItem(Header header2, boolean z) {
            this.overwrite = z;
            this.header = header2;
        }
    }
    ...
    public void addHeaders(List<Header> list) {
        ...
        for (Header headerItem : list) {
            this.headers.add(new HeaderItem(headerItem));
        }
    }
    ...
    public void addQueryStringParameter(List<NameValuePair> list) {
        ...
        if (list != null && list.size() > 0) {
            for (NameValuePair add : list) {
                this.queryStringParams.add(add);
            }
        }
    }
    ...
    public HttpEntity getEntity() {
        ...
        if (this.fileParams != null && !this.fileParams.isEmpty()) {
            HttpEntity multipartEntity = new MultipartEntity(HttpMultipartMode.STRICT, (String) null, Charset.forName(this.charset));
            if (this.bodyParams != null && !this.bodyParams.isEmpty()) {
                for (NameValuePair next : this.bodyParams) {
                    try {
            ...
            for (Map.Entry next2 : this.fileParams.entrySet()) {
                multipartEntity.addPart((String) next2.getKey(), (ContentBody) next2.getValue());
            }
    ...
    public void setHeaders(List<Header> list) {
        ...
        for (Header headerItem : list) {
            this.headers.add(new HeaderItem(headerItem, true));
        }
    }
